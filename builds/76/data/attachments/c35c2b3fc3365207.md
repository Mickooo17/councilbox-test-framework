...
- Name: tests/loginTests.spec.ts >> LoginPage - UI Tests >> Verify that "/login" page contains all fields and buttons @XR-2280 @smoke @regression
- Location: tests/loginTests.spec.ts:77:5

# Error details

```
Error: Username input field should be visible
...
Call log:
  - Username input field should be visible locator('#username') with timeout 5000ms
